...
외부 PDF 관찰기는 정확한 승인 호스트·HTTPS·인증서 검증·리디렉션 허용 목록을 유지한다. 클라우드에서 직접 DNS 사전 조회만 `EAI_AGAIN`으로 실패하면 `dns_preflight_unavailable`을 기록하고, 네트워크 설정을 바꾸지 않은 표준 HTTPS 전송을 시도한다. 이는 IP 사전 검증 성공을 뜻하지 않는다. DNS가 사설 IP를 반환하거나 비어 있는 경우, 다른 DNS 오류, HTTPS 접근 거부·인증서 오류는 그대로 차단한다. 실제 전체 PDF 바이트 취득에 성공한 경우에만 관측 해시를 기록한다.
